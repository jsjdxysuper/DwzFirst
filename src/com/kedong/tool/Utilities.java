...
public class Utilities {
	...
	public static String getSelectDate(int daynum) {
		if (daynum > 0) {
			String startdate = getToday();
			String nowdate = startdate;
			
			for (int i = 0; i < daynum; i++) {
				nowdate = getTomorrow(nowdate);
			}
	 ...
	 * @return
	 */
	public static String strDateFormatTrans(String lineDateForm){
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		SimpleDateFormat sdfNoLine = new SimpleDateFormat("yyyyMMdd");
		Date date = new Date();
		try {
			date = sdf.parse(lineDateForm);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		String strDateNoLine = sdfNoLine.format(date);
		return strDateNoLine;
	}
	 ...
	 * @return
	 */
	public static int getDaysInOneMonth(int year,int month){
		Calendar time = Calendar.getInstance();
		time.clear();
		time.set(Calendar.YEAR, year);
		time.set(Calendar.MONTH, month-1);
		int dayCount = time.getActualMaximum(Calendar.DAY_OF_MONTH);
		return dayCount;
	}
}